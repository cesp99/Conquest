...
/// is inserted.
pub fn enabled_brackets(language: &str, tree: &Tree, text: &Rope, offset: usize) -> u64 {
    let Some(entry) = registry().get(language) else {
        return u64::MAX;
    };
    let Some(overrides) = entry.overrides() else {
        return u64::MAX;
    };
    let Some(scope) = scope_at(overrides, tree, text, offset) else {
        return u64::MAX;
    };
