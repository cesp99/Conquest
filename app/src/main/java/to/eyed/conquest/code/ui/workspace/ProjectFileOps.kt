 ...
 * [to.eyed.conquest.code.core.ProjectEntry.path] uses, and every one of them is
...
object ProjectFiles {
     ...
     *
     * Rejecting rather than sanitizing, as [to.eyed.conquest.code.core.ProjectsRoot]
    ...
    fun moveInto(root: File, rel: String, destDir: String): FileOpResult {
        val source = resolve(root, rel) ?: return notInProject(rel)
        val target = resolve(root, destDir) ?: return notInProject(destDir)
        ...
        val name = freeName(target, source.name, source.isDirectory)
        return try {
            Files.move(source.toPath(), File(target, name).toPath())
            FileOpResult.Done(join(destDir, name))
        } catch (e: IOException) {
