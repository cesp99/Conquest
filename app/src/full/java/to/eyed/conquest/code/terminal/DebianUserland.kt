 ...
 *
 * This only compiles into the `full` flavour, which targets SDK 28: Android
 * refuses to execute downloaded programs at any newer target, and every binary
 * `apt` installs is a downloaded program.
 */
...
private object DebianUserland : UserlandBackend {
    ...
    private fun marker(context: Context) = File(rootfs(context), ".conquest-userland")
    ...
    private fun configure(context: Context, root: File) {
        ...
        File(root, "etc/hostname").writeText("conquest\n")
        ...
        File(root, "etc/apt/apt.conf.d/99conquest").writeText(
            """
            Acquire::Retries "3";
            DPkg::Use-Pty "0";
            Dir::Log::Terminal "";
        ...
        File(root, "root/.bashrc").writeText(
            """
            # Conquest Code — Debian userland. Edit freely; this file is yours.
