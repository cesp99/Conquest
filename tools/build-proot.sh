...
ndk_version="$(sed -n 's/^conquest\.ndkVersion=//p' "$repo_root/gradle.properties")"
...
# CONQUEST PATCH: clang 18+ makes implicit function declarations an error, and
...
if ! grep -q "CONQUEST PATCH" "$ashmem"; then
    sed -i '0,/#include <stdlib.h>/s//#include <stdlib.h>\n#include <string.h>     \/* CONQUEST PATCH: strcmp\/memset *\//' "$ashmem"
fi
